...
##### 不同的锁
* 共享锁(shared lock)：行级别的共享锁，也叫读锁，简称S锁。
* 排他锁(exclusive lock)：行级别的排他锁，也叫写锁，简称X锁。
* 意向共享锁(intention shared lock)：表级别的共享锁，简称IS锁。
* 意向排他锁(intention exclusive lock)：表级别的排他锁，简称IX锁。
...
* 可串行化(SERIALIZABLE)：在这种隔离级别下，如果事务A和事务B同时对一张表进行读写/写读/写写操作，则一方会阻塞直到另一方全部完成。不会出现任何错误。实现方式为：事务在读取数据时，必须先对其加表级共享锁 ，直到事务结束才释放；但事务在更新数据时，必须先对其加表级排他锁，直到事务结束才释放。
